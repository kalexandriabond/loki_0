...
fig_dir = os.path.join(home, 'Dropbox/loki_0/simple_rt_experiment_probabilityC/analysis/figures/')

av_manifold_df = pd.read_csv(os.path.join(home,'Dropbox/loki_0/simple_rt_experiment_probabilityC/analysis/aggregated_data/av_est.csv'))
...
def create_epoch_window_number(session_data):
    ...
    for epoch_number in session_data.epoch_number.unique()[1:]:  # only get the second epoch onward (otherwise no prior trial to include)
        ...
        # session_data.iat(epoch_window_start_idx, 'epoch_window_number', # set_value deprecated since 0.21.0
        # session_data.iloc[epoch_start_idx].epoch_number)

        session_data.iloc[epoch_window_start_idx].epoch_window_number = session_data.iloc[epoch_start_idx].epoch_number
...
# reg_window_df_v2.to_csv(os.path.join(home, 'Dropbox/loki_0/simple_rt_experiment_probabilityC/analysis/aggregated_data/av_est.csv'), index=False)
#
...
av_manifold_df.to_csv(os.path.join(home, 'Dropbox/loki_0/simple_rt_experiment_probabilityC/analysis/aggregated_data/av_est.csv'), index=False)

# angle distributions (in radians) calculated from the z-scored a-v estimates
g = sns.FacetGrid(data=av_manifold_df, col='subj_id', col_wrap=4)
g = g.map(plt.hist, 'theta_radians_z')
plt.savefig(os.path.join(fig_dir,'theta_radians_z.png'))

# radius distributions calculated from the z-scored a-v estimates
g = sns.FacetGrid(data=av_manifold_df, col='subj_id', col_wrap=4)
g = g.map(plt.hist, 'r_z')
plt.savefig(os.path.join(fig_dir, 'r_z.png'))

# angle distributions (in degrees) calculated from the z-scored a-v estimates
g = sns.FacetGrid(data=av_manifold_df, col='subj_id', col_wrap=4)
g = g.map(plt.hist, 'theta_deg_z')
plt.savefig(os.path.join(fig_dir,'theta_deg_z.png'))
