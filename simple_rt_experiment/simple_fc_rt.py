...
if testing:
    ...
    condition = str(0)
    session = str(0)
    exp_param_file = exp_param_directory + 'low_volatility_low_conflict.csv'
else:
...
window = visual.Window(size = screen_size, units='pix', monitor = mbp_monitor, color = [-1,-1,-1], \
       ...
       False,fullscr=False, pos=center)
...
max_rt = .7
min_rt = .1
left_pos_x = [-screen_size[0]/2]
right_pos_x = [screen_size[0]/2]
l_r_y = 0
...
l_r_x_arr = np.concatenate((l_x, r_x),0)
shuffle(l_r_x_arr)
...
def ForcedChoiceTrial(max_reward_idx,t):
    ...
    HighValueFractal.setPos([l_r_x_arr[t], l_r_y])
    LowValueFractal.setPos([-l_r_x_arr[t],l_r_y])
...
for i in range(0,n_trials):
    ...
    choice,rt = ForcedChoiceTrial(max_reward_idx[i],i)
    rt_list.append(rt)

    # if choice == 'left': #need to give high reward identifier, get position of high rew & key mapping
    ...
    right_pos = [right_pos_x[0],0]
    choice_emphasis.pos = right_pos
    rewardMsg.text = str('+' + str(reward_t2[i]))
